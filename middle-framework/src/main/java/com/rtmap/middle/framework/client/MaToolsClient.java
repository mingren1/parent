...
@Component
@FeignClient(
        url = "${feign.ma.mptools}",
        name = "toolsUrl",
        configuration = FeignClientConfiguration.class,
        fallback = MaToolsClientFallback.class
)
public interface MaToolsClient {
    /**
     * 根据id获取渠道详情
     * <p>
    ...
    Result<WxChannelDetailVO> getChannelDetailById(
    ...
    @Headers("Content-Type: application/json;charset=UTF-8")
    Result<WxAppinfoVO> getMiniAppDetail(
    ...
    @Headers("Content-Type: application/json;charset=UTF-8")
    Result<WxAccessTokenDTO> getAccessToken(
            @NotNull(message = "tenantId 不能为空")
            @RequestParam("tenantId") Long tenantId,
            @NotNull(message = "tenantType 不能为空")
    ...
    @Headers("Content-Type: application/json;charset=UTF-8")
    Result<WxAccessTokenDTO> refreshAccessToken(
            @NotNull(message = "tenantId 不能为空")
            @RequestParam("tenantId") Long tenantId,
            @NotNull(message = "tenantType 不能为空")
            @RequestParam("tenantType") Integer tenantType,

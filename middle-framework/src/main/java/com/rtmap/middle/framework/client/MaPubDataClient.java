...
@Component
@FeignClient(
        url = "${feign.ma.rts_pubdata_server}",
        name = "pubDataUrl",
        configuration = FeignClientConfiguration.class,
        fallback = MaPubDataClientFallback.class
)
public interface MaPubDataClient {
    ...
    Result<PubShopDetailVO> getShopListByMarketId(
    ...
    @Headers("Content-Type: application/json;charset=UTF-8")
    Result<List<PubShopInfoSimpleVO>> getShopsByKeyWords(
            @RequestParam("marketId") Long marketId,
    ...
    @Headers("Content-Type: application/json;charset=UTF-8")
    Result<List<PubFloorInfoVO>> getFloorListByMarketId(
            @NotNull(message = "商场ID 不能为空")
            @RequestParam("marketId") Long marketId,
    ...
    @Headers("Content-Type: application/json;charset=UTF-8")
    Result<List<PubShopInfoSimpleVO>> getShopList(
            @NotNull(message = "商户搜索类型不能为空")
            @RequestParam("searchType") String searchType,
            @NotNull(message = "商场ID不能为空")
            @RequestParam("marketId") Long marketId,
            @RequestParam("searchChar") String searchChar
    );
    ...
    @Headers("Content-Type: application/json;charset=UTF-8")
    Result<PubMarketInfoVO> getMarketInfo(
            @NotNull(message = "商场ID不能为空")
            @RequestParam("marketId") Long marketId);
    ...
    @Headers("Content-Type: application/json;charset=UTF-8")
    Result<PubShopSummaryVO> getShopInfo(
            @NotNull(message = "商场ID不能为空")
    ...
    @Headers("Content-Type: application/json;charset=UTF-8")
    Result<PubShopInfoVO> getShopPositionInfo(
            @NotNull(message = "商场ID不能为空")
            @RequestParam("shopId") Long shopId);

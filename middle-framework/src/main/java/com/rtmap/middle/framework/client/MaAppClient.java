...
import com.rtmap.middle.framework.client.fallback.MaAppClientFallback;
...
@Component
@FeignClient(
        url = "${feign.ma.rtmap-app}",
        name = "appMarketUrl",
        configuration = FeignClientConfiguration.class,
        fallback = MaAppClientFallback.class
)
public interface MaAppClient {
    ...
    Result<List<ApplyShopBean>> getActivityShopList(@NotBlank(message = "活动ID不能为空")
    ...
    Result<ActivityDeatilVO>  getActivityDetail(@NotBlank(message = "活动ID不能为空")
                                                            @RequestParam("activityId") String activityId);
    /**
     * 活动组列表查询
     * <p>
    ...
    Result<List<ActivityGroupVO>> activityGroupList(@RequestParam(value = "portalId") Long portalId,
                                                    @RequestParam(value = "subjectType") Integer subjectType,
